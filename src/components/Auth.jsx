    ...
    <div className={styles.overlay}>
      <div className={styles.card}>
        ...
        <div className={styles.tabs}>
          ...
          <button className={`${styles.tab} ${tab === 'signup' ? styles.activeTab : ''}`} onClick={() => { setTab('signup'); setError(null); setMessage(null) }}>Sign Up</button>
        </div>
        ...
        <input className={styles.input} type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleEmailSubmit()} />

        {tab !== 'reset' && (
          <>
            <label className={styles.label}>Password</label>
            <input className={styles.input} type="password" placeholder={tab === 'signup' ? 'Min. 8 characters' : '••••••••'} value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleEmailSubmit()} />
          </>
        )}

        <button className={styles.btnPrimary} onClick={handleEmailSubmit} disabled={loading}>
          {loading ? 'Please wait...' : tab === 'reset' ? 'Send reset email' : tab === 'login' ? 'Log In' : 'Create Account'}
        </button>

        {tab === 'login' && (
          <p className={styles.footerText}>
            <span className={styles.footerLink} onClick={() => { setTab('reset'); setError(null); setMessage(null) }}>
              Forgot password?
            </span>
          </p>
        )}
        ...
        <p className={styles.footerText}>
          {tab === 'reset' ? (
            <span className={styles.footerLink} onClick={() => { setTab('login'); setError(null); setMessage(null) }}>
              Back to log in
            </span>
          ) : (
            <>
              {tab === 'login' ? "Don't have an account? " : 'Already have an account? '}
              <span className={styles.footerLink} onClick={() => { setTab(tab === 'login' ? 'signup' : 'login'); setError(null); setMessage(null) }}>
